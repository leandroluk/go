// schema/object/rule/comparator_gtecsfield.go
package rule

import (
	"github.com/leandroluk/go/v/internal/ast"
	"github.com/leandroluk/go/v/internal/engine"
)

type gteCSFieldComparator struct {
	code string
	path string
}

func GteCSField(code string, path string) Comparator {
	return gteCSFieldComparator{code: code, path: path}
}

func (c gteCSFieldComparator) Apply(context *engine.Context, root ast.Value, child ast.Value) bool {
	otherValue := ast.Query(root, c.path)

	order, ok := compareOrder(child, otherValue)
	if ok && order >= 0 {
		return false
	}

	return context.AddIssueWithMeta(c.code, "must be greater or equal", map[string]any{
		"path":   c.path,
		"actual": astValueToMeta(child),
		"value":  astValueToMeta(otherValue),
	})
}
