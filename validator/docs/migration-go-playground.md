# Migração do go-playground/validator para validator

No `go-playground/validator` você descreve validação em **tag**. No validator você descreve em **Go**.
...
| Tag (validator.v10) | Equivalente no validator                  | Notas                                                                        |
| ------------------- | ----------------------------------------- | ---------------------------------------------------------------------------- |
...
| Tag                                  | Equivalente no validator (text)               | Exemplo                                              |
| ------------------------------------ | --------------------------------------------- | ---------------------------------------------------- |
...
validator.Object(func(u *User, s *validator.ObjectSchema[User]) {
	s.Field(&u.Name, func(ctx *validator.Context, v any) (any, bool) {
		return validator.Text().Required().Min(3).Max(50).ValidateAny(v, ctx.Options)
	})
	...
	s.Field(&u.Role, func(ctx *validator.Context, v any) (any, bool) {
		return validator.Text().OneOf("admin", "user", "guest").ValidateAny(v, ctx.Options)
	})

	s.Field(&u.Tags, func(ctx *validator.Context, v any) (any, bool) {
		itemSchema := validator.Text().Min(2).Max(20)
		return validator.ArraySchemaOf[string]().
			Max(10).
			Items(func(itemCtx *validator.Context, item any) (any, bool) {
...
emailsByKey := validator.RecordSchemaOf[string]().
	Min(1).
	Max(20).
	Keys(validator.Text().Pattern("^[a-zA-Z0-9]+$")).
	Values(func(ctx *validator.Context, v any) (any, bool) {
		return validator.Text().Required().Email().ValidateAny(v, ctx.Options)
	})
```
...
No validator isso vira uma destas opções:
